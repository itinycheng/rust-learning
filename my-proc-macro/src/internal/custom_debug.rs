...
use syn::{DeriveInput, Attribute, Meta, MetaNameValue};
use syn::Data;
use syn::DataStruct;
use syn::Fields;
use quote::quote;
...
pub(crate) fn parse(ast: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let fields = if let Data::Struct(DataStruct {
        ...
        return Err(syn::Error::new(Span::call_site(), "get struct's fields failed"));
    };
    ...
    let debug_fields = fields.iter().map(|field| {
        ...
        let (rename, format) = attrs.into_iter().map(|attr| parse_attr(attr))
...
fn parse_attr(attr: &Attribute) -> (String, String) {
    ...
    if meta_list.path.segments[0].ident == "debug" {
        meta_list.nested.iter().for_each(|nested_meta| {
            if let NestedMeta::Meta(Meta::NameValue(MetaNameValue { path, lit: Lit::Str(value), .. })) = nested_meta {
